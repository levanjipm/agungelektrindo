...
<?php
		...
		$sql			= "SELECT code_purchaseorder.id FROM code_purchaseorder JOIN purchaseorder ON purchaseorder.purchaseorder_id = code_purchaseorder.id
						WHERE name LIKE '%" . $term . "' AND purchaseorder.status = '0' 
						UNION (SELECT DISTINCT(purchaseorder.purchaseorder_id) as id FROM purchaseorder
						JOIN itemlist ON itemlist.reference = purchaseorder.reference
						WHERE purchaseorder.reference LIKE '%" . $term . "%' OR  itemlist.description LIKE '%" . $term . "%' AND purchaseorder.status = '0')
						UNION
