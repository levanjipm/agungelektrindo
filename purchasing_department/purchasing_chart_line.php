<?php
	...
	$sql			= "SELECT MONTH(purchases.date) as month, YEAR(purchases.date) as year FROM purchases 
						GROUP BY YEAR(purchases.date) DESC, MONTH(purchases.date) DESC LIMIT 10 OFFSET 1";
	...
	while($row		= $result->fetch_assoc()){
		...
		$sql_value_ae		= "SELECT SUM(purchases.value / 1000000) as value FROM purchases 
								WHERE MONTH(purchases.date) = '$month' AND YEAR(purchases.date) = '$year'";
...
<h3 style='font-family:bebasneue'>Purchasing history</h4>
