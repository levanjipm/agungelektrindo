...
<?php
	$sql_table 							= "SELECT id,bank_opponent_id,label,value,transaction,date, isdone, isdelete
											FROM code_bank WHERE date >= '" . $start_date . "' AND date <= '" . $end_date . "' AND major_id = '0' ORDER BY date ASC, id ASC";
	$result_table 						= $conn->query($sql_table);
	while($table 						= $result_table->fetch_assoc()){
		$label 							= $table['label'];
		$transaction_done_status		= $table['isdone'];
		$transaction_delete_status		= $table['isdelete'];
		...
		?>
			...
			<td>
				<button class='<?php if($transaction_done_status == 0 || $transaction_delete_status == 0){ echo 'button_danger_dark';} else { echo 'button_success_dark'; } ?>' <?php if($transaction_done_status == 0 || $transaction_delete_status == 0){ echo 'disabled';} ?>><i class='fa fa-pencil'></i></button>
			</td>
		</tr>
<?php
	}
