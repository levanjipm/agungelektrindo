<?php
	...
	if($validation == true){
		...
		$sql_insert = "INSERT INTO code_sales_return (submission_date,customer_id,do_id,reason,guid) 
					VALUES (NOW(),'$customer_id','$do_id','$reason','$guid')";
		echo $sql_insert;
		$conn->query($sql_insert);
		
		$sql_select 	= "SELECT id FROM code_sales_return ORDER BY ID DESC LIMIT 1";
		...
		foreach($return_quantity_array as $return_quantity){
			...
			if($return_quantity > 0){
				$sql		= "INSERT INTO sales_return (quantity, return_code, delivery_order_id)
								VALUES ('$return_quantity','$return_id','$key')";
				$conn->query($sql);
			}
			echo $key;
			next($return_quantity_array);
