...
<body>
	<div class="row">
		...
		<div class="col-sm-8" id="printable">
			...
			<?php
				$sql_q = "SELECT COUNT(*) AS sum FROM quotation WHERE quotation_code = '" . $q_id . "'";
				...
				<table class="table">
					...
					<?php 
						...
						while($row = $result->fetch_assoc()){
							...
							$total_price[$i] = $row['total_price'];
							
							$sql_desc = "SELECT * FROM itemlist WHERE reference = '" . $ref[$i] . "'";
					...
					<tfoot>
					<?php
					$sql_t = "SELECT * FROM  code_quotation WHERE name = '" . $q_id . "'";
					...
					?>
						<tr>
							...
							<td style="text-align:center">Rp.<?= number_format($value,2) ?></td>
						</tr>
						<tr>
