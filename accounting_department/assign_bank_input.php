<?php
	...
	$transaction_status	= $bank['isdone'];
	...
	if($isdone			== 0){
		if($transaction == 2){ //Kredit ->masuk duit//
			...
			if($will_be_paid == 0){
				...
				$sql_get_code_bank_id 		= "SELECT id FROM code_bank WHERE transaction = '1' ORDER BY id DESC LIMIT 1";
				$result_get_code_bank_id 	= $conn->query($sql_get_code_bank_id);
				$get_code_bank_id 			= $result_get_code_bank_id->fetch_assoc();
				
				$new_bank_id 				= $get_code_bank_id['id'];
			}
